...
If you have access to the codebase (running in a repo context):

**First, read the project conventions:**
1. **Read `AGENTS.md`** (if it exists in repo root) - Contains project-specific coding standards, architecture overview, module naming conventions, and testing procedures
2. **Read `.clio/instructions.md`** (if it exists) - Contains methodology and workflow conventions
3. Use these conventions as your baseline for evaluating the PR
...
4. **Compare with existing patterns** - If the PR adds a new operation/command, look at how existing operations are implemented in the SAME file. Does the new code follow the same patterns? Flag deviations from established patterns, not deviations from your personal preferences.
